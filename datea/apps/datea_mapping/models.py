...
from datea_category.models import DateaCategory, DateaFreeCategory
...
class DateaMapping(DateaAction):
    ...
    information_destiny = models.TextField(
        _("What happens with the data?"),
        max_length=500,
    ...
    report_success_message = models.TextField(
        _("Item submitted success message"),
        blank=True,
        null=True,
        max_length=140,
    ...
    default_color = models.CharField(
        _('Default Item Color'), max_length=7, blank=True, default="#ff9c00",
        help_text=_("Default color for map items (used when there's no categories defined)."))
    # ZONES
    #zones = models.ManyToManyField(Zone, blank=True, null=True, default=None)

    # CATEGORIES / VARIABLES
    item_categories = TreeManyToManyField(
        DateaFreeCategory,
...
class DateaMapItem(models.Model):
    ...
    images = models.ManyToManyField(DateaImage, verbose_name=_('Images'),
                                    null=True, blank=True,
                                    related_name="map_item_images")

    # location
    position = models.PointField(_('Position'), blank=True, null=True,
                                 spatial_index=False)
    address = models.CharField(_('Address'), max_length=255, blank=True,
                               null=True)

    # relation to mapping object: UPDATE -> refer to mapping as an action.
    # More generic!!
    ...
    # category
    category = TreeForeignKey(DateaFreeCategory, verbose_name=_("Category"),
